...
generally consists of first simulating some genetic
variation, and then simulating quantitative traits based on it.
The combined genetic variation and
simulated quantitative traits then represent truth-data
which association testing methods can be evaluated against.
Simulating genetic variation is
...
Roughly speaking, model-based
simulation methods provide better control of
population processes such as demography but can be overly simplistic,
whereas resampling methods better capture some of the nuance of
real data, while lacking a direct connection to population genetics models.
...
have been successfully used to model large-scale GWAS
cohorts~\cite[e.g.][]{martin2017,zaidi2020}.

An important part of these simulation methods is that they
output ARGs using
the ``succinct tree sequence'' encoding~\citep{wong2023general}
and the open-source \texttt{tskit} library~\citep{ralph2020}.
A key benefit of this approach is that it allows us to
store enormous genetic variation datasets compactly.
Instead of storing the genetic sequence variation in the conventional
